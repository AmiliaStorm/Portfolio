...
// The portfolio's old CSS makes this mount fill the entire viewport.
// Contain it here so SAGE cannot obscure the page, even if that CSS
// is still cached. Inline styles override the old #sageAvatarMount rule.
...
// Updated once per frame in animate() — the page-space pixel position
// of her core, so sage.js can put the clickable hit-zone and speech
// bubble exactly where she visually is instead of guessing.
let lastScreenPosition = null;
const screenProjectPos = new THREE.Vector3();
...
window.SAGEAvatar = {
  ...
  getObject3D: () => sage,
  // Site integration: sage.js positions the clickable hit-zone and
  // speech bubble to follow her actual rendered position, since she
  // now flies freely instead of being placed by sage.js's own logic.
  // Returns viewport pixel coordinates, or null before the first
  // frame has rendered.
  getScreenPosition: () => lastScreenPosition,
  ...
  forcePhase: (name) => {
    phaseBlendTarget.flutterAmp =
      name === "perch" ? 0.05 : name === "glide" ? 0.16 : 1;
...
function pickNextPhase(t) {
  ...
  if (r < 0.55) {
    flightPhase = "flutter";
    phaseDuration = rand(4.5, 9.5);
    phaseBlendTarget.flutterAmp = 1;
    phaseBlendTarget.foldAmount = 0;
  } else if (r < 0.86) {
    flightPhase = "glide";
    phaseDuration = rand(1.3, 2.6);
    phaseBlendTarget.flutterAmp = 0.16;
    phaseBlendTarget.foldAmount = 0;
  } else {
    flightPhase = "perch";
    phaseDuration = rand(1.2, 2.4);
    phaseBlendTarget.flutterAmp = 0.05;
